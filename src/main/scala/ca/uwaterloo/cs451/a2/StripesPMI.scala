...
/**
  ...
  */
  ...
  class StripesPMIPartitioner(partitions: Int) extends Partitioner {
  ...
  object StripesPMI extends Tokenizer {
    ...
    def main(argv: Array[String]) {
      ...
      val conf = new SparkConf().setAppName("StripsPMI")
      ...
      .sortByKey()
      .collectAsMap()
      // get a map of occurance of each words Map("a"->1, "b"->2, "c"->3,.....)

      val stripes = textFile.flatMap(
        line => {
          val tokens = tokenize(line)
          if (tokens.length > 0){
      ...
      .map(pair => (pair._1._1, (pair._1._2, pair._2))) // from ((a,b), 12) to (a, (b, 2))
      .groupByKey()

      // stripe = ('a',List(('b',1),('c',2),...))
      val pmis = stripes.map(
        stripe =>(
          stripe._1, // key
          stripe._2.map(
            key_count =>(
              ...
              key_count._1 +"="+ (
                Math.log10(
                  key_count._2.toFloat * numOfLines / (words.get(stripe._1).get * words.get(key_count._1).get)
                ),
                key_count._2
              )
            )
