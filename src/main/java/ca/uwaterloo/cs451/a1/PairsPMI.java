...
public class PairsPMI extends Configured implements Tool {
  ...
  private static final class MyMapper extends Mapper<LongWritable, Text, PairOfStrings, FloatWritable> {
    ...
    public void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException {
      // same as words in WordCound 
      List<String> tokens = Tokenizer.tokenize(value.toString());

      if (tokens.size() < 2)
        return;
      Set<String> tokens_showing = new HashSet<String>();
      // up to 40 of the 
      for (int i = 1; i < Math.min(40, tokens.size()); i++){
        tokens_showing.add(tokens.get(i));
      }
      System.out.print(tokens_showing);
      // for (int i = 1; i < tokens.size(); i++) {
      //   //  co-occurance of i-1 and i token
      //   BIGRAM.set(tokens.get(i - 1), tokens.get(i));
      //   context.write(BIGRAM, ONE);
      //   // occurance of ith token
  ...
  private static final class MyReducer extends Reducer<PairOfStrings, FloatWritable, PairOfStrings, FloatWritable> {
